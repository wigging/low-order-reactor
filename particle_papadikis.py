...
from funcKinetics import papadikis
...
sa = 1.879e-7    # surface area of particle for DF = 400 um, m^2
v = 5.553e-12    # volume of particle for DF = 400 um , m^3

#sa = 4.614e-6   # surface area of particle for DF = 2.8 mm, m^2
#v = 4.011e-10   # volume of particle for DF = 2.8 mm, m^3
...
wood = np.zeros((nt, m))        # create array for wood density
char = np.zeros((nt, m))        # create array for char density
gas = np.zeros((nt, m))         # create array for gas density
tar = np.zeros((nt, m))         # create array for tar density
...
for i in range(1, nt):
    ...
    # kinetic reactions
    wood[i], gas[i], tar[i], char[i], g = papadikis(wood[i-1], gas[i-1], tar[i-1], char[i-1], T[i], H, dt)
